...
for(let k in experiences){
  let experience = experiences[(experiences.length - k) - 1];
  let content = '';
  let meta = [
    '---'
  ];
  ...
  meta.push(yamljs.stringify(experience, 4));
  meta.push('---');
  meta.push(content.split('<br/>').join('\n\n'));
  let key = k < 10 ? `0${k}` : k;
  fs.writeFileSync(`${__dirname}/content/experiences/${key}.md`, meta.join('\n'));
}
